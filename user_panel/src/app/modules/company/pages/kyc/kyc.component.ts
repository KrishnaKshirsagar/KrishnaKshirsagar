...
export class KycComponent implements OnInit {
      ...
      switch (event.target.id) {
        ...
        case "banking":
          ...
          for (var i = 0; i < event.target.files.length; i++) {
            this.bankingFile.push(event.target.files[i]);
          }
          this.bankingUrl = [];
          if (this.bankingFile) {
  ...
  ngOnInit(): void {
    ...
    this.verify_form = this.fb.group({
      ...
      mobileVerify: this.fb.group({
        ...
        mobile_verification_otp: ["", [Validators.required,Validators.pattern]],
      }),
  ...
  getKYCdetails() {
    this.apiService.getkyc(this.companyid).subscribe((r: any) => {
      if (r && r.status == true) {
        ...
        this.cheque_image_flag = r && r.data && r.data.chequeImages && r.data.chequeImages.files ? true : false 
        ...
        this.storeDetails = r.data && r.data.storeImages && r.data.storeImages.files ? r.data.storeImages : false;
        this.chequeDetails = r.data && r.data.chequeImages && r.data.chequeImages.files ? r.data.chequeImages : false;
  ...
  AadharCaptured_data(){
    var formData = new FormData();
      formData.append("company_id", sessionStorage.getItem("companyid"));
  ...
  verifypan() {
    ...
    body.append("company_id", sessionStorage.getItem("companyid"));
  ...
  { 
    ...
    if (
      ...
      { 
        ...
        this.businessfile.forEach((element) => {
        ...
        this.apiService.kycRequest(form).subscribe((res: any) => {
          if (res && res.status == true) {
            ...
            if (this.bankingFile.length == 1) {
              ...
              this.snackBar.open(
                "Banking documents uploaded successfully",
                "Close",
                {
                  duration: 5 * 3000,
                  panelClass: ["error-dialog"],
  ...
  uploadFinancialGstDoc(){
    ...
    if ( this.document_form.value 
      ...
      {
        this.financialUrl.forEach((element) => {
          form.append("FinancialDetails", element, element.name);
        });
        this.gstUrl.forEach((element) => {
